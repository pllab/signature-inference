...
object Helpers {
  ...
  def alloc( σ:Store, a: Address, bv: BValue ): Store = 
    σ alloc (a, bv) 
      ...
      (acc, a) ⇒ {
       ...
       case Clo(ρc, m @ Method(self, args, s)) ⇒
              ...
              if ( notJS.Mutable.pruneStore ) { // yes, prune store
                ...
                // !! PDG 
                notJS.Mutable.pdg.addCFGEdge(τ, τ1, CallingControl)
                val ka = τ1.toAddr
    ...
    val (bv1, σ1) = sorts.foldLeft( (BValue.⊥, σ) )(
      (acc, sort) ⇒ sort match {
        ...
        case DStr ⇒
          ...
          (acc._1 ⊔ bv2, σ2 putObj( bv2.as.head, o1, Str.NumStr ⊔ Str.α("length")))
  ...
  def updateObj(bv1:BValue, bv2:BValue, bv3:BValue, σ:Store): (Option[(BValue, Store)], Option[EValue]) = {
    ...
    val noexc =
      if ( isStrong ) {
        ...
        // TODO: make the hint more precise?            
  ...
  def refineExc(e:Exp, σ:Store, ρ:Env, ß:Scratchpad, bvf:BVFilter) = {
    // !! PDG
    notJS.Mutable.pdg.pauseRecording()
    val res = refine(bvf, e, σ, ρ, ß)
    // !! PDG
    notJS.Mutable.pdg.resumeRecording()
    (res._1, res._2)
  }
    ...
    case Binop(⌜⋆⌝, el, er) ⇒
      ...
      refineableAddrObj(objbv, strbv.str, σ) match {
        case Some((addr, o)) ⇒ {
          ...
          val σT = σ.putObjStrong(addr, oT, strbv.str)
          val oF = o copy (extern = o.extern ++ (strbv.str -> newBVF))
          val σF = σ.putObjStrong(addr, oF, strbv.str)
          (σT, ß, σF, ß)
        }
        case None ⇒ (σ, ß, σ, ß)
      }
  ...
  def refineableAddrObj(bv:BValue, str:Str, σ:Store): Option[(Address, Object)] = {
    ...
    if ((bv.as.size == 1) && (σ.isStrong(bv.as.head)) && (Str.isExact(str))) {
      val obj = σ.getObj(bv.as.head, str)
