...
object InitArrays {
  ...
  val Array_Obj = createInitFunctionObj(Native(
    (selfAddr: BValue, argArrayAddr: BValue, x: Var, ρ: Env, σ: Store, ß:Scratchpad, κ: KStack, τ:Trace) ⇒ {
      ...
      val argsObj = σ.getObj(argArrayAddr.as.head, Str.α("length") ⊔ Str.NumStr)
      ...
      val notones = if (argLenMaybeNot1 || arg0MaybeNotNumeric) {
        ...
        makeState(Address.inject(arrayAddr), x, ρ, σ1.putObj(arrayAddr, updatedArrObj, Str.⊤), ß, κ, τ)
      } else Set()
  ...
  val Array_prototype_join_Obj = createInitFunctionObj(Native(
    (selfAddr: BValue, argArrayAddr: BValue, x: Var, ρ: Env, σ: Store, ß:Scratchpad, κ: KStack, τ:Trace) ⇒ {
      ...
      if (lenVal == Num.inject(Num.α(0)))
        ...
        val summaryVal = lookup(selfAddr.as, Str.NumStr, σ)
  ...
  val Array_prototype_pop_Obj = createInitFunctionObj(Native(
    (selfAddr: BValue, argArrayAddr: BValue, x: Var, ρ: Env, σ: Store, ß:Scratchpad, κ: KStack, τ:Trace) ⇒ {
      ...
      if (arrayAddrs.size != selfAddr.as.size)
        ...
        val summaryVal = lookup(arrayAddrs, Str.NumStr, σ)
        val updatedArrObj = newArray(NReal, List(), Some(summaryVal), oldArrayObj, true)
        val nonzeroς =
          if (lenVal.n.def0)
            Set()
          else
            makeState(summaryVal, x, ρ, σ.putObj(arrayAddr, updatedArrObj, Str.⊤), ß, κ, τ)
    ...
    (selfAddr: BValue, argArrayAddr: BValue, x: Var, ρ: Env, σ: Store, ß:Scratchpad, κ: KStack, τ:Trace) ⇒ {
      ...
      val σ1 = arrayAddrs.foldLeft(σ) {
        case (acc, arrayAddr) ⇒ {
          ...
          // TODO: precision if length known?
          val summaryVal = lookup(Addresses(arrayAddr), Str.NumStr, acc) ⊔ lookup(argArrayAddr.as, Str.NumStr, acc)
          val updatedArrObj = newArray(NReal, List(), Some(summaryVal), oldArrayObj, true)

          if (isStrong)
            acc.putObjStrong(arrayAddr, updatedArrObj, Str.⊤)
          else   
            acc.putObjWeak(arrayAddr, updatedArrObj, Str.⊤)
        }
      }    
  ...
  val Array_prototype_concat_Obj = createInitFunctionObj(
    Native(
      (selfAddr: BValue, argArrayAddr: BValue, x: Var, ρ: Env, σ: Store, ß:Scratchpad, κ: KStack, τ:Trace) ⇒ {
        ...
        val argsArray = σ getObj(argArrayAddr.as.head, Str.NumStr)

        val argsSummary = selfAddr ⊔ argsArray(Str.NumStr).getOrElse(BValue.⊥)
        ...
        val entrySummary = argsSummary.copy(as = nonArrayAddrs) ⊔ lookup(arrayAddrs, Str.NumStr, σ)
        ...
        makeState(resaddr_bv, x, ρ, σ1.putObj(resaddr, resArray, Str.⊤), ß, κ, τ)
      }
    ), Map("length" → Num.inject(Num.α(1))))
    ...
    Native(
      (selfAddr: BValue, argArrayAddr: BValue, x: Var, ρ: Env, σ: Store, ß:Scratchpad, κ: KStack, τ:Trace) ⇒ {
        ...
        if (arrayAddrs.size != selfAddr.as.size)
          ...
          val selfArray = σ getObj(arrayAddr, Str.NumStr)
          val entrySummary = selfArray(Str.NumStr) getOrElse Undef.BV
  ...
  val Array_prototype_splice_Obj = createInitFunctionObj(
    Native(
      (selfAddr: BValue, argArrayAddr: BValue, x: Var, ρ: Env, σ: Store, ß:Scratchpad, κ: KStack, τ:Trace) ⇒ {
        ...
        assert(argArrayAddr.as.size == 1,
          "Array.prototype.concat: Arguments array refers to multiple addresses")
        val argArray = σ getObj(argArrayAddr.as.head, Str.NumStr)
        ...
        if (arrayAddrs.size != selfAddr.as.size)
          ...
          val oldSelf = σ getObj(selfArrayAddr, Str.NumStr)
          val oldSummary = oldSelf(Str.NumStr) getOrElse Undef.BV
          val newSummary = oldSummary ⊔ argArray(Str.NumStr).getOrElse(BValue.⊥)

          /* update self with summary including arguments */
          val newSelf = newArray(NReal, List(), Some(newSummary), oldSelf, true)
          val σ1 = σ.putObj(selfArrayAddr, newSelf, Str.⊤)
          ...
          val σ3 = σ2.putObj(retAddr, retArray, Str.⊤)
  ...
  val Array_prototype_reverse_Obj = createInitFunctionObj(Native(
    (selfAddr: BValue, argArrayAddr: BValue, x: Var, ρ: Env, σ: Store, ß:Scratchpad, κ: KStack, τ:Trace) ⇒ {
      ...
      if (arrayAddrs.size != selfAddr.as.size) makeState(typeError, x, ρ, σ, ß, κ, τ)
        ...
        makeState(Addresses.inject(arrayAddrs), x, ρ, σ.putObj(arrayAddr, updatedArrObj, Str.⊤), ß, κ, τ)
      }
    }
