...
case class Env( env:Map[PVar, Addresses] = Map() )  extends SmartHash {
  ...
  // add a single new binding
  def +( bind: (PVar, Address) ): Env = {
    Env( env + (bind._1 → Addresses(bind._2)) )
  }
...
case class Store( a2v:Map[Address, BValue] = Map(), 
  ...
  def alloc( bind:List[(Address,BValue)] ): Store = {
    ...
    // strong writes
    bindn.foreach {
      case (a, _) ⇒ Mutable.pdg.addToWriteSet(a, Str.⊤, true)
    }
    bindw.foreach {
      case (a, _) ⇒ Mutable.pdg.addToWriteSet(a, Str.⊤, false)
    }
    
    Store( a2v ++ bindn ++ w_a2v, a2o, a2k, weak ++ bindw.unzip._1.toSet )
  }

  // allocate a single value at the given address, perform 
  // abstract counting
  def alloc( a: Address, bv: BValue ): Store = {
    val alreadyAllocd = a2v contains a
    if (alreadyAllocd) {
      val newv = a2v(a) ⊔ bv
      // !! PDG
      // weak writes
      Mutable.pdg.addToWriteSet(a, Str.⊤, false)
      this copy (a2v = a2v + (a → newv), weak = weak + a) 
    } else {
      // !! PDG
      // strong writes
      Mutable.pdg.addToWriteSet(a, Str.⊤, true)
      this copy (a2v = a2v + (a → bv))
    }
  }
  ...
  def apply( as:Addresses ): BValue = {
    assert( as.nonEmpty )
    // !! PDG
    Mutable.pdg.addToReadSet(as, Str.⊤, as.size == 1 && isStrong(as.head))
  ...
  def apply( a: Address ): BValue = {
    // !! PDG
    Mutable.pdg.addToReadSet(a, Str.⊤, isStrong(a))
    a2v get a match {
      case Some(bv) ⇒ bv
  ...
  // retrieve an object
  def getObj( a:Address, shint: Str ): Object = {
    // !! PDG
    Mutable.pdg.addToReadSet(a, shint, isStrong(a) && Str.isExact(shint))
    a2o get a match {
      case Some(o) ⇒ o
      case None ⇒ throw Store.AddrNotFound
    }
  ...
  def +( av:(Addresses,BValue), shint: Str = Str.⊤ ): Store = {
    ...
    // !! PDG
    Mutable.pdg.addToWriteSet(av._1, shint, av._1.size == 1 && isStrong(av._1.head))
    val as = av._1
    val _a2v = 
  ...
  def putObj( a:Address, o:Object, shint: Str ): Store = 
    if ( weak(a) ) putObjWeak( a, o, shint )
    else putObjStrong( a, o, shint )
  
  
  // replace an object using a strong update
  def putObjStrong( a:Address, o:Object, shint: Str ): Store = {
    // !! PDG
    Mutable.pdg.addToWriteSet(a, shint, true)
  ...
  def putObjWeak( a:Address, o:Object, shint: Str ): Store = {
    // !! PDG
    Mutable.pdg.addToWriteSet(a, shint, false)
...
case class Scratchpad( mem:IndexedSeq[BValue] ) extends SmartHash {
  ...
  def apply( x:Scratch ): BValue = {
    // !! PDG
    Mutable.pdg.addToReadSet(x)
    mem(x.n)
  }
  
  def update( x:Scratch, bv:BValue ): Scratchpad = {
    // !! PDG
    Mutable.pdg.addToWriteSet(x)
...
sealed abstract class Str extends SmartHash {
  def ⊔( str:Str ): Str =
    (this, str) match {
      ...
      case (SConstNotNum(str1), SConstNotNum(str2)) ⇒
        Str.hack_⊔(str1, str2)
      case (_:SPrefix, SNum) | (SNum, _:SPrefix) | (_:SPrefix, _:SConstNum) |
           (_:SConstNum, _:SPrefix) ⇒ STop
      case (_:SPrefix, SNotNum) | (SNotNum, _:SPrefix) ⇒ SNotNum
      case (SPrefix(str1), SPrefix(str2)) ⇒
        Str.hack_⊔(str1, str2)
      case (SPrefix(str1), SConstNotNum(str2)) ⇒
        Str.hack_⊔(str1, str2)
      case (SConstNotNum(str1), SPrefix(str2)) ⇒
        Str.hack_⊔(str1, str2)
      case _ ⇒ SNum
    }
  ...
  def ⊑( str:Str ): Boolean =
    (this, str) match {
      ...
      case (_:SPrefix, SNotNum) ⇒ true
      case (SPrefix(str1), SPrefix(str2)) ⇒ str1 startsWith str2
      case (SConstNotNum(str1), SPrefix(str2)) ⇒ str1 startsWith str2
      case _ ⇒ false
    }
  ...
  def ≡( str:Str ): Bool =
    (this, str) match {
      ...
      case (_:SPrefix, _:SConstNum) | (_:SConstNum, _:SPrefix) |
           (SNum, _:SPrefix) | (_:SPrefix, SNum) ⇒ Bool.False
      case (SPrefix(str1), SPrefix(str2)) ⇒
        if ( (str1 startsWith str2) || (str2 startsWith str1) ) Bool.⊤
        else Bool.False
      case (SPrefix(str1), SConstNotNum(str2)) ⇒
        if ( str2 startsWith str1 ) Bool.⊤ else Bool.False
      case (SConstNotNum(str1), SPrefix(str2)) ⇒
        if ( str1 startsWith str2 ) Bool.⊤ else Bool.False
  ...
  def ++( str:Str ): Str = 
    (this, str) match {
      ...
      case (pre:SPrefix, _) ⇒ pre
      case (SConstNotNum(pre), STop) ⇒ if (pre=="") SNotNum else SPrefix(pre)
      ...
      case (SConstNotNum(str1), SConstNotNum(str2)) ⇒ SConstNotNum(str1 + str2)
      case (SConstNotNum(str1), SConstNum(str2)) ⇒ SConstNotNum(str1 + str2)
      case (SConstNum(str1), SConstNotNum(str2)) ⇒ SConstNotNum(str1 + str2)
      case (SConstNotNum(str1), SPrefix(str2)) ⇒ SPrefix(str1 + str2)
      case (SConstNum(str1), SPrefix(str2)) ⇒ SPrefix(str1 + str2)
      case _ ⇒ SNotNum
    }
  ...
  def ≺( str:Str ): Bool = 
    (this, str) match {
      ...
      case (SConstNum(str1), SConstNum(str2)) ⇒ Bool.α(str1 < str2)
      case (SPrefix(str1), SPrefix(str2)) ⇒
        if ( (str2 startsWith str1) || (str1 startsWith str2) ) Bool.⊤
        else Bool.α(str1 < str2)
      case (SPrefix(str1), SConstNotNum(str2)) ⇒
        if ( str2 startsWith str1 ) Bool.⊤ else Bool.α(str1 < str2)
      case (SPrefix(str1), SConstNum(str2)) ⇒
        if ( str2 startsWith str1 ) Bool.⊤ else Bool.α(str1 < str2)
      case (SConstNotNum(str1), SPrefix(str2)) ⇒
        if ( str1 startsWith str2 ) Bool.⊤ else Bool.α(str1 < str2)
      case (SConstNum(str1), SPrefix(str2)) ⇒
        if ( str1 startsWith str2 ) Bool.⊤ else Bool.α(str1 < str2)
      case _ ⇒ Bool.⊤
    }
  ...
  def ≼( str:Str ): Bool =
    (this, str) match {
      ...
      case (SPrefix(str1), SConstNum(str2)) ⇒
        if ( str2 startsWith str1 ) Bool.⊤ else Bool.α(str1 < str2)
      case (SConstNotNum(str1), SPrefix(str2)) ⇒
        if ( str1 != str2 && (str1 startsWith str2) ) Bool.⊤
        else Bool.α(str1 <= str2)
      case (SConstNum(str1), SPrefix(str2)) ⇒
        if ( str1 != str2 && (str1 startsWith str2) ) Bool.⊤
        else Bool.α(str1 <= str2)
  ...
  def toNum: Num =
    this match {
      case SBot ⇒ Num.⊥
      case STop | SNum ⇒ Num.⊤
      case SNotNum | _:SPrefix | _:SConstNotNum ⇒ Num.NaN
...
// a prefix of a SNotNum
case class SPrefix( str:String ) extends Str {
  assert( str != "" ) // use SNotNum instead if str is ""
  override def toString = str + " · Σ*"
}
...
object Str {
  ...
  lazy val Empty = SConstNotNum("")
  // single character string
  lazy val SingleChar = STop
  // string representation of a date
  lazy val DateStr = SNotNum
  // string representation of a function
  lazy val FunctionStr = SNotNum

  def hack_⊔(a: String, b: String): Str = {
    val pick = (a, b) match {
      case (_, "undefined") ⇒ Some(a)
      case (_, "null") ⇒ Some(a)
      case ("undefined", _) ⇒ Some(b)
      case ("null", _) ⇒ Some(b)
      case _ ⇒ None
    }
    pick match {
      // if any is "undefined" or "null", drop it and pick the other 
      case Some(s) ⇒ if (s == "") SNotNum else SPrefix(s)
      case None ⇒ 
        val pre = Str.gcp(a, b)
        if ( pre == "" ) SNotNum else SPrefix(pre)
    }
  }
  ...
  // compute greatest common prefix of two strings
  def gcp( s1:String, s2:String ): String =
    if ( s1 == "" || s2 == "" || s1(0) != s2(0) ) ""
    else s1(0) + gcp( s1.drop(1), s2.drop(1) )
}
...
case class Object( extern:ExternMap, intern:Map[Str, Any], present:Set[Str] ) extends SmartHash {
      ...
      val intern1 = for ( (k, v) ← o.intern ) yield k match {
        ...
        case `domain` ⇒ {
          val me = intern get (k) match {
            case Some(mv) ⇒ mv.asInstanceOf[Str]
            case None ⇒ Str.⊥
          }
          val that = v.asInstanceOf[Str]
          (k, me ⊔ that)  
        }
        case bv ⇒
  ...
  // get this object's (it must be a XHR object) domain
  def getDomain: Str = 
    if (intern.contains(domain))
      intern(domain).asInstanceOf[Str]
    else Str.⊥
...
case class ExternMap( top:Option[BValue] = None, 
  ...
  def exactlte( str:Str ): Set[Str] = 
    str match {
      ...
      case SNotNum | _:SPrefix ⇒
        exactnotnum.keySet.map(_.asInstanceOf[Str])

      case SNum ⇒
        exactnum.keySet.map(_.asInstanceOf[Str])
...
case class WhileK( e:Exp, s:Stmt, w: While ) extends Kont
case class ForK( bv:BValue, x:Var, s:Stmt, f: For ) extends Kont
...
case class KStack( κs:List[Kont], exc:List[Int] = List(0) ) extends SmartHash {
  def ⊔( rhs:KStack ): KStack = {
    ...
    val newks = (κs zip rhs.κs) map {
      ...
      case (ForK(bv1, x1, s1, f1), ForK(bv2, x2, s2, f2)) ⇒
        assert( x1 == x2 && s1 == s2 && f1 == f2)
        ForK(bv1 ⊔ bv2, x1, s1, f1)
  ...
  // what is the next trace we would get  
  // assuming a normal control flow
  def getNextTraceSet(τ: Trace, σ: Store, local: Boolean): Set[Trace] = {
    if (!notJS.Mutable.pdg.condition)
      Set[Trace]()
    else {      
      top match {
        case SeqK(s :: _) ⇒ Set(τ update s)
        case SeqK(_) ⇒ this.pop.getNextTraceSet(τ, σ, local)
        case WhileK(_, _, w) ⇒ this.pop.getNextTraceSet(τ update w, σ, local)
        case ForK(_, _, _, f) ⇒ this.pop.getNextTraceSet(τ update f, σ, local)
        case RetK(x, ρ, isctor, τc) ⇒ {
          val σ1 = 
            if (notJS.Mutable.pruneStore)
              σ ⊔ PruneStoreToo(τc)._1
            else σ  
          this.pop.getNextTraceSet(τ update τc, σ1, local)
        }
        case TryK( _, _, s3 ) ⇒ this.repl(FinK(Set(Undef.BV))).getNextTraceSet(τ update s3, σ, local)
        case CatchK( s3 ) ⇒ this.repl(FinK(Set(Undef.BV))).getNextTraceSet(τ update s3, σ, local)
        case FinK( vs ) ⇒ vs flatMap {
          case _: BValue ⇒ this.pop.getNextTraceSet(τ, σ, local)
          // TODO: verify this is correct
          case _ ⇒ Set[Trace]()
        }
        case LblK( _ ) ⇒ this.pop.getNextTraceSet(τ, σ, local)
        case AddrK( a, _ ) ⇒ 
          if (local) Set[Trace]()
          else (σ getKont a).flatMap(_.getNextTraceSet(τ, σ, local))
        case HaltK ⇒ Set()  
      }
    }
  }
