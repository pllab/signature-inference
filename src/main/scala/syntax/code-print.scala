...
object CodePrint {
  ...
  def writeText(a: AST, reverseMapping: Map[Int, String], file: String, annotations: Map[Int, String] = Map()): Unit = {
    val out = if (file == null)
      new PrintWriter(System.out)
    else
      new PrintWriter(new BufferedWriter(new FileWriter(file)))

    out.write(getText(a, reverseMapping, annotations))
  ...
  def getText(a: AST, reverseMapping: Map[Int, String], annotations: Map[Int, String] = Map()): String = {
    var outputString = ""
    ...
    def printNode(depth: Int, id: String, label: String) {
      ...
      val suffix = if(fpart.length< 110) " "*(110-fpart.length) else ""
      val annotation = annotations.getOrElse(id.toInt, "")
      outputString += fpart + suffix + "ID: " + id + " " + annotation + "\n"
    }
